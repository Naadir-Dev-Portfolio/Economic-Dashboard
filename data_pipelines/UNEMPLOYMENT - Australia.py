#!/usr/bin/env python3
"""
Metric: Australia Unemployment Rate

Definition:
  The unemployment rate measures the percentage of the Australian labor force 
...
Importance:
  ...
  • Sharp increases (>1 percentage point over six months) often precede economic downturns.

When to worry:
  If unemployment exceeds ~6% or jumps by more than 1 percentage point within six months,
  it indicates broader economic stress and potential recession risk.
"""
...
SERIES       = "LRHUTTTTAUQ156S"  # FRED code for Australia unemployment rate
START_DATE   = "1970-01-01"
END_DATE     = date.today().strftime("%Y-%m-%d")
OUT_DIR      = "data"
OUT_FILE     = os.path.join(OUT_DIR, "aus_unemployment_rate.xlsx")
SHEET_NAME   = "Unemployment_AUS"
...
try:
    ...
    print(f"❌ Failed to fetch Australia unemployment: {e}")
    sys.exit(1)
...
with pd.ExcelWriter(OUT_FILE, engine="xlsxwriter", datetime_format="yyyy-mm-dd") as writer:
    ...
    chart.set_title   ({"name": "Australia Unemployment Rate"})
